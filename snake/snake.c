...
void turn_snake(int new_direction) {
  if (snake_direction == UP && new_direction != DOWN) { // if new direction is not the opposite 
    snake_direction = new_direction;                    // of the current direction, turn the snake
  } else if (snake_direction == DOWN && new_direction != UP) {
    snake_direction = new_direction;
  } else if (snake_direction == LEFT && new_direction != RIGHT) {
    snake_direction = new_direction;
  } else if (snake_direction == RIGHT && new_direction != LEFT) {
    snake_direction = new_direction;
  }
 ...
 */
void grow_snake() {
    snake[snake_length].x = snake[snake_length - 1].x; // set the new part of the body to have
    snake[snake_length].y = snake[snake_length - 1].y; // the same position as the last part
    snake_length += 1;
}
...
int move_snake() {
  int i;
  int x1 = snake[0].x; // values for previous x and y
  int y1 = snake[0].y;
  int x2; // values for current x and y
  int y2;
  for (i = 1; i < snake_length; i++) {
    x2 = snake[i].x; // setting snake[i] to snake[i - 1]
    y2 = snake[i].y;
    snake[i].x = x1;
    snake[i].y = y1;
    x1 = x2;
...
void spawn_fruit() {
  ...
  int valid = 1;
  while (TRUE) {
    int x = rand16() % COLUMNS; // generate a random location for fruit
    int y = rand16() % ROWS;
    for (i = 0; i < bombs_count; i++) { // check if there is a bomb at the location
      if (x == bomb[i].x && y == bomb[i].y) {
        valid = 0;
      }
    }
    if (valid) { // if not, spawn the fruit, otherwise, keep looping
      fruit.x = x;
      fruit.y = y;
      return;
    }
...
void spawn_bomb() {
  ...
  while (TRUE) {
    int x = rand16() % COLUMNS; // generate a random location for bomb
    int y = rand16() % ROWS;
    for (i = 0; i < bombs_count; i++) { // check if it a bomb already exists at location
      if (x == bomb[i].x && y == bomb[i].y) {
        valid = 0;
      }
    }
    if (x == fruit.x && y == fruit.y) { // check if fruit exists at location
      valid = 0;
    }
    if (valid) { // if not, spawn bomb, otherwise keep looping
...
int update_game_state() {
    int collision;
    int move = move_snake();
    if (move == 0) { // if move is invalid, game is lost
      return GAME_LOST;
    }
    ...
    if (collision == BOMB || collision == SNAKE) {
      ...
      if (snake_length % 5 == 0) {
        spawn_bomb(); // create bomb whenver snake has a length that is a multiple of 5
        lc4_puts ((lc4uint*)"Bomb spawned\n") ;
...
void draw_snake() {
    ...
    for (i = 0; i < snake_length; i++) {
      draw_pixel(snake[i].y, snake[i].x, color);
    }
}
...
void draw_bombs() {
    ...
    int i;
    for (i = 0; i < bombs_count; i++) {
      draw_pixel(bomb[i].y, bomb[i].x, color);
    }
}
...
void draw_fruit() {
  ...
  draw_pixel(fruit.y, fruit.x, color);
}
...
void play_game() {
    ...
    while (state == GAME_PLAYING) {
        ...
        if (state == GAME_WON) {
            lc4_puts ((lc4uint*)"Game won!\n");
            return;
        } else if (state == GAME_LOST) {
            lc4_puts ((lc4uint*)"Game lost.\n");
            return;
        }
    }
}
